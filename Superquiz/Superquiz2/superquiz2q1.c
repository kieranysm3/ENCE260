...
/*
    while (precedes(student, Current) != 1) {
        if (precedes(student, Head) == 1) {
            //my student is smaller than head so it should be new head
            student->next = Head;
            Head = student;
            return Head;
        } else if (Current->next == NULL) {             //my student is greater than head check and my student is now largest in the array
            // while student has not been added to list check
            //i am the biggest add me here
            if (Previous == NULL) {
                Head->next = student;
                return Head;

            } else if { student->next = Head;
                Head = student;
            Previous->next = student;
            student->next = Current;
            return Head;
    ...
    //student is less than current
    Previous->next = student;
    student->next = Current;


    //i am smaller than next guy so put me before him


        //i am at the front of the list alr
        //while not found i update current to the right and repeat
    return Head;
}
*/
...
{
    FILE* inputFile = fopen(argv[20], "r");
    if (inputFile == NULL) {
